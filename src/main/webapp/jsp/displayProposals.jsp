...
<ul>
	...
	<br>
	<%
		List proposalList = (List) request.getAttribute("proposalList");
	%>
	...
	<table>
		...
		<%
			for (int i = 0; i < proposalList.size(); i++) {
				ProposalRequest e = (ProposalRequest) proposalList.get(i);
		%>
		<tr>
			<td><%=e.getProposalId()%></td>
			<td><%=e.getTraineeId()%></td>
			<td><%=e.getTraineeName()%></td>
			<td><a href="/accept?proposalId=<%=e.getProposalId()%>">Accept</a></td>
			<td><a href="/reject?proposalId=<%=e.getProposalId()%>">Reject</a></td>
</tr>
		<% }%>
