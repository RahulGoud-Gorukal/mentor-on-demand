<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	...
	import="java.util.*,com.example.MentorOnDemand.Model.Mentor,com.example.MentorOnDemand.Model.ProposalRequest"%>
			...
			<tr>
				...
				<%
					if (session.getAttribute("userName") != null) {
						...
						{%>
							<td><a
							href="requestMentor?mentorId=<%=m.getMentorId()%>&username=<%=session.getAttribute("userName")%>&technology=<%=m.getTechnology()%>&mentorName=<%=m.getMentorName()%>">Request</a></td>
						<%}
						else
						{
						for(int j=0;j<statusList.size();j++)
						{
							ProposalRequest pr=(ProposalRequest)statusList.get(j);
							if(pr.getProposalStatus().equalsIgnoreCase("Accepted")&& pr.getMentorId()==m.getMentorId())
							{	
								%>
